...
window.Vaadin.Flow['_vaadintheme_my-theme1_globalCss'] = window.Vaadin.Flow['_vaadintheme_my-theme1_globalCss'] || [];
export const applyTheme = (target) => {
  
  const injectGlobal = (window.Vaadin.Flow['_vaadintheme_my-theme1_globalCss'].length === 0) || (!window.Vaadin.Flow['_vaadintheme_my-theme1_globalCss'].includes(target) && target !== document);
  if (injectGlobal) {
    injectGlobalCss(stylesCss.toString(), target);
    
    window.Vaadin.Flow['_vaadintheme_my-theme1_globalCss'].push(target);
  }
  if (!document['_vaadintheme_my-theme1_componentCss']) {
    
    document['_vaadintheme_my-theme1_componentCss'] = true;
  }
